...
if (isset($_GET['type']) && !empty($_GET['type'])) {
    ...
    switch ($type) {
        case '1':
            $res = sendPostRequest('https://st.rndpwd.info/generate-password', ['HTTP_REFERER' => $_SERVER['HTTP_REFERER']]);
            break;
        case '2':
            $res = sendPostRequest('https://st.rndpwd.info/copy-password', ['HTTP_REFERER' => $_SERVER['HTTP_REFERER']]);
            break;
    }
    if (!empty($res)) {
